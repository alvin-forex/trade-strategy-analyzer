...
// EA Name Abbreviation Map
var EA_DISPLAY_ABBREV = {
    'DragonWave': 'DW', 'Dragon Wave': 'DW',
    'MKD Pro': 'MKDPro', 'MKD': 'MKD',
    'SMA Pro': 'SMAPro', 'SMA': 'SMA',
    'Flash': 'Flash', 'S10': 'S10',
    'Gemini Client': 'GC', 'Gemini Server': 'GS',
    'StableHelper': ''
};

function abbreviateEaName(name) {
    if (!name) return '';
    var lower = name.toLowerCase();
    // Match longest first to avoid partial matches
    var keys = Object.keys(EA_DISPLAY_ABBREV).sort(function(a, b) { return b.length - a.length; });
    for (var i = 0; i < keys.length; i++) {
        if (lower.indexOf(keys[i].toLowerCase()) >= 0) return EA_DISPLAY_ABBREV[keys[i]];
    }
    // Fallback: strip version
    return name.replace(/\s*v?[\d.]+$/, '').trim();
}
...
function renderSetInfo(setConfigs) {
    ...
    for (const cfg of setConfigs) {
        if (cfg.lot_mode === 'copy_trade') {
            parts.push(`<span style="color:var(--pri)">${abbreviateEaName(cfg.ea_name)}</span> Copy: LV${cfg.start_lv}-${cfg.end_lv}`);
            ...
            parts.push(`<span style="color:var(--pri)">${abbreviateEaName(cfg.ea_name)}</span> Lots: ${lotsStr} | PipStep: ${psStr}`);
        } else if (cfg.lot_mode === 'fixed' && cfg.lots && cfg.lots.length === 1) {
            parts.push(`<span style="color:var(--pri)">${abbreviateEaName(cfg.ea_name)}</span> Fixed: ${cfg.lots[0].toFixed(2)} lots`);
        } else {
            parts.push(`<span style="color:var(--pri)">${abbreviateEaName(cfg.ea_name)}</span>`);
        }
    }
    html += parts.join(' | ');
    html += '</div>';
